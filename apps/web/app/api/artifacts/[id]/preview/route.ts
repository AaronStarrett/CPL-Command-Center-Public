import { createArtifactPreviewResponse } from "@bea/artifacts";
...
export async function GET(
  ...
  const accessDenial = await requirePersistedArtifactAccess(context, id, "preview");
  if (accessDenial !== null) {
    return finalizeArtifactRouteResponse(accessDenial, context.correlationId);
  }
  const response = await createArtifactPreviewResponse(context.store, {
    artifactId: id,
    audit: context.audit,
