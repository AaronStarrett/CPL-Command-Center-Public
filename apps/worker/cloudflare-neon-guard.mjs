// Private prototype. No default network implementation, environment lookup, or credentials.
...
export function connectionPolicy(connectionString, policy) {
  try {
    if (
      ...
      // eslint-disable-next-line no-control-regex -- Intentionally reject control characters and spaces in connection input.
    ...
    if (
      ...
      reject();
    // eslint-disable-next-line no-control-regex -- Decoded passwords must also exclude control characters and spaces.
  ...
  const guardedFetch = async (url, options) => {
    ...
    try {
      ...
      if (
        ...
        names.join(",") !==
          "neon-array-mode,neon-batch-isolation-level,neon-connection-string,neon-raw-text-output" ||
