...
export function parsePreviewStopArguments(arguments_) {
  ...
  throw new Error("Usage: preview-stop.mjs [--check]");
}
...
async function requirePortsReleased(dependencies = {}) {
  ...
  if (unavailable.length > 0) {
    throw new Error(
      `Preview ports remain occupied (${unavailable.join(", ")}); no process was touched.`,
    );
  }
  return results;
}
...
async function handleStoppedLaunchLock(options, toolchain, dependencies = {}) {
  ...
  if (inspection.state === "live") {
    if (options.check) {
      emit(dependencies, "BEA_PREVIEW_STOP_CHECK=BLOCKED (LAUNCH_STARTING)\n");
      return { outcome: "BLOCKED", readOnly: true, reason: "LAUNCH_STARTING" };
    }
    throw new Error("An exact Preview launcher is still starting; no process was touched.");
  }
  ...
  if (options.check) {
    emit(dependencies, "BEA_PREVIEW_STOP_CHECK=BLOCKED (STALE_LAUNCH_LOCK)\n");
  ...
  if (!recovered) {
    throw new Error("Stale Preview launch-lock ownership changed; evidence was preserved.");
  }
  emit(
    dependencies,
    "Removed a validated dead-owner Preview launch lock after all fixed ports were free; no process was terminated.\nBEA_PREVIEW_STOP=STALE_LAUNCH_LOCK_CLEANED\n",
  );
  ...
  return (dependencies.waitFor ?? waitFor)(
    async () => {
      ...
      if (currentMetadata.metadata) {
        ...
        throw new Error("Preview metadata became invalid during shutdown and was preserved.");
      }
    ...
    },
    {
      ...
      timeoutMessage:
        "The authenticated Preview supervisor did not stop and release its exact ports within 40 seconds; no broad kill was attempted.",
...
export async function stopPreview(options = {}, dependencies = {}) {
  ...
  if ((dependencies.platform ?? process.platform) !== "win32") {
    throw new Error("The isolated Preview stop tool supports Windows only.");
  }
  (dependencies.assertPaths ?? assertPreviewPathContract)(dependencies.paths ?? previewPaths);
  ...
  if (decision.action === "block-unverifiable") {
    throw new Error(
      "Preview metadata or the tracked supervisor identity is unverifiable; no process was touched.",
    );
  }
  if (decision.action === "already-stopped") {
    ...
    emit(
      dependencies,
      options.check
        ? "BEA_PREVIEW_STOP_CHECK=PASS (ALREADY_STOPPED)\n"
        : "BEA_PREVIEW_STOP=ALREADY_STOPPED\n",
    );
    return { outcome: "ALREADY_STOPPED", readOnly: options.check === true };
  }
  if (decision.action === "report-stale") {
    emit(dependencies, "BEA_PREVIEW_STOP_CHECK=BLOCKED (STALE_METADATA)\n");
  ...
  if (decision.action === "report-running") {
    emit(
      dependencies,
      `BEA_PREVIEW_STOP_CHECK=PASS (RUNNING:${tracked.status.state}, PID:${String(tracked.metadata.supervisorOsIdentity.pid)})\n`,
    );
  ...
  emit(
    dependencies,
    "Stopped the exact launcher-owned Preview supervisor, web, and worker processes; ports 3100/3101/3102 are released.\nBEA_PREVIEW_STOP=PASS\n",
  );
  return { outcome: "PASS" };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  void main().catch((error) => {
    process.stderr.write(
      `BEA_PREVIEW_STOP=BLOCKED (${error instanceof Error ? error.message : "UnknownError"})\n`,
    );
    process.exitCode = 1;
  });
}
