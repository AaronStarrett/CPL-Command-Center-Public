import { createArtifactDownloadResponse } from "@bea/artifacts";
...
export async function GET(
  ...
  const accessDenial = await requirePersistedArtifactAccess(context, id, "download");
  if (accessDenial !== null) {
    return finalizeArtifactRouteResponse(accessDenial, context.correlationId);
  }
  const response = await createArtifactDownloadResponse(context.store, {
    artifactId: id,
    audit: context.audit,
