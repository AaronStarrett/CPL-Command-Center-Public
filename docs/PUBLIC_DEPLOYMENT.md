...
Status: hosting adapter and bounded workflow IMPLEMENTED. Commit `30bb0ef` was PUBLISHED and deployed for acceptance on September 21, 2026; anonymous HTTPS checks passed, but observed CPU exceeded the Free budget and the attempted Google callback returned 401. Live owner workflow acceptance is BLOCKED. The subsequent custom dispatcher requires a new reviewed release and deployed measurements. This source does not establish a production service or automatic deployment connection.
...
Cloudflare CLI authorization, a dedicated free PostgreSQL project, and the Google OAuth client have been prepared. The client has the exact HTTPS origin and callback, only OpenID/email/profile scopes, and the owner as its sole test user; credentials remain outside source. PostgreSQL migration, restricted-role, tenant, queue, and disposable backup/restore checks have executed; their scope is recorded in [hosted test coverage](HOSTED_TEST_COVERAGE.md). The attempted owner sign-in has not passed; physical device verification remains NOT RUN. An adapter build or synthetic signed assertion does not satisfy them.
...
On the authorized Windows SSD, invoke these commands with the verified Node executable: `& 'D:\Cyber Pirate Labs\93_TOOLS_AND_CACHE\CPL-Command-Center\toolchain\node-v24.19.0-win-x64\node.exe' scripts/cloudflare-hosting.mjs build web`. Use that same executable for `dry-run` and `deploy`; the launcher preserves its directory first in the child process search path even when Windows supplies both `Path` and `PATH`.
...
The [hosting compatibility record](HOSTING_COMPATIBILITY.md) documents the observed CPU blocker and the `workers.dev` suitability boundary. A fictional acceptance deployment must not be described as business-critical hosting. Successful owner-operated Google sign-in is still required; physical passkeys and deployed organization/lead/proposal behavior remain NOT RUN. Runtime binding verification alone does not establish those workflows. Customer sending, paid AI calls, billing activation, DNS changes, and legacy tenant migration remain outside this bounded release.
