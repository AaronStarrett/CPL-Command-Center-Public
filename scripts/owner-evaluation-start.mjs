...
import {
  ...
  createRunLogPaths,
  ensureOwnerEvaluationDirectories,
  ...
  ownerEvaluationBrowserSignInUrl,
  openAppendOnlyLog,
...
export function parsePreviewStartArguments(arguments_) {
  ...
  for (const argument of arguments_) {
    ...
    else throw new Error("Usage: owner-evaluation-start.mjs [--check] [--no-open] [--seed]");
  }
...
export function readPreviewInitialization(options = {}) {
  const path = options.path ?? ownerEvaluationPaths.initializationFile;
  ...
  try {
    ...
    throw new Error("Owner Evaluation initialization marker is invalid.");
  }
  if (
    value?.version !== 1 ||
    value.profile !== "owner-evaluation" ||
    ...
    throw new Error(
      "Owner Evaluation initialization marker does not match this canonical checkout.",
    );
  }
...
export function writePreviewInitialization(existing, seeded, dependencies = {}) {
  ...
  const value = {
    ...
    profile: "owner-evaluation",
    repositoryRoot,
    version: 1,
  };
  (dependencies.writeInitializationFile ?? writeJsonAtomic)(
    dependencies.path ?? ownerEvaluationPaths.initializationFile,
    value,
    dependencies,
  );
  return value;
...
function writeInitialMetadata(metadata, dependencies = {}) {
  ...
  (dependencies.writeInitialMetadata ?? writeJsonExclusive)(
    dependencies.metadataPath ?? ownerEvaluationPaths.metadataFile,
    metadata,
    dependencies,
  );
}
...
async function verifyPortsAvailable(dependencies = {}) {
  ...
  if (unavailable.length > 0) {
    throw new Error(
      `Owner Evaluation ports are occupied (${unavailable.join(", ")}); no process was touched.`,
    );
  }
  return results;
}
    ...
    return (dependencies.spawn ?? spawn)(toolchain.nodeExecutable, [previewSupervisorScript], {
      ...
      env: {
        ...(dependencies.childEnvironment ??
          createOwnerEvaluationChildEnvironment(dependencies.environment ?? process.env)),
...
export async function terminateExactSpawnedPreviewSupervisor(tracked, dependencies = {}) {
  ...
  if (
    ...
    throw new Error(
      "Pending Owner Evaluation supervisor launch evidence is invalid; no kill was sent.",
    );
  }
  ...
  try {
    ...
    throw new Error(
      "Pending Owner Evaluation supervisor OS identity could not be captured; no kill was sent.",
    );
  }
  if (!captured || !pathsEqual(captured.executablePath, tracked.executable)) {
    throw new Error(
      "Pending Owner Evaluation supervisor executable identity is invalid; no kill was sent.",
    );
  }
  const confirmed = await inspect(tracked.pid);
  if (!confirmed || !processIdentityMatches(captured, confirmed)) {
    throw new Error("Pending Owner Evaluation supervisor identity changed; no kill was sent.");
  }
  ...
  if (!beforeEscalation || !processIdentityMatches(captured, beforeEscalation)) {
    throw new Error(
      "Pending Owner Evaluation supervisor identity changed before escalation; no kill was sent.",
    );
  }
  tracked.process.kill("SIGKILL");
  if (!(await waitForExit(tracked.process, 5_000))) {
    throw new Error("The exact spawned Owner Evaluation supervisor process did not stop.");
  }
}
  ...
  return (dependencies.waitFor ?? waitFor)(
    async () => {
      ...
      if (!pathsEqual(identity.executablePath, expectedExecutable)) {
        throw new Error("Spawned Owner Evaluation supervisor does not use canonical BEA Node.");
      }
      return identity;
    },
    {
      intervalMilliseconds: 100,
      timeoutMessage: "The Owner Evaluation supervisor OS identity was not observable.",
...
function createLaunchMetadata(
  ...
  return {
    ...
    controlPort: ownerEvaluationPorts.control,
    controlToken,
    instanceId,
  ...
  return (dependencies.waitFor ?? waitFor)(
    async () => {
      if (!(dependencies.processIsAlive ?? processIsAlive)(supervisorPid)) {
        throw new Error("The owned Owner Evaluation supervisor exited before readiness.");
      }
      const tracked = await (dependencies.inspectTracked ?? inspectTrackedPreview)({
        path: ownerEvaluationPaths.metadataFile,
      });
      ...
      if (tracked.state === "running" && tracked.status.state === "failed") {
        throw new Error("The owned Owner Evaluation supervisor reported readiness failure.");
      }
      if (["invalid", "stale"].includes(tracked.state)) {
        throw new Error("Owner Evaluation supervisor identity became unverifiable during startup.");
      }
    ...
    },
    {
      ...
      timeoutMessage: "The Owner Evaluation runtime was not ready within 185 seconds.",
...
async function waitForConcurrentPreviewReady(initial, dependencies = {}) {
  const result = await (dependencies.waitFor ?? waitFor)(
    async () => {
      ...
      return {
        error: new Error(
          `The concurrent Owner Evaluation supervisor entered ${tracked.status.state}; no browser was opened.`,
        ),
      };
    },
    {
...
export async function startPreview(options = {}, dependencies = {}) {
  ...
  if ((dependencies.platform ?? process.platform) !== "win32") {
    throw new Error("The Owner Evaluation launcher supports Windows only.");
  }
  const paths = dependencies.paths ?? ownerEvaluationPaths;
  (dependencies.assertPaths ?? assertOwnerEvaluationPathContract)(paths);
  ...
  if (decision.action === "block-running-seed") {
    throw new Error("Stop the running Owner Evaluation before using --seed.");
  }
  if (decision.action === "block-unverifiable") {
    throw new Error(
      "Owner Evaluation metadata or the tracked process identity is unverifiable; no process was touched.",
    );
  }
  if (decision.action === "block-failed") {
    throw new Error(
      "The authenticated Owner Evaluation supervisor reported failed startup; no browser was opened.",
    );
  }
  if (decision.action === "block-stopping") {
    throw new Error(
      "The authenticated Owner Evaluation supervisor is stopping; no browser was opened.",
    );
  }
  if (decision.action === "wait-for-ready") {
    if (options.check) {
      emit(dependencies, "BEA_OWNER_EVALUATION_START_CHECK=BLOCKED (RUNNING:starting)\n");
      return { outcome: "BLOCKED", readOnly: true, reason: "STARTING" };
    }
    const ready = await waitForConcurrentPreviewReady(tracked, dependencies);
    emit(
      dependencies,
      `BEA Owner Evaluation finished its concurrent startup under authenticated supervisor PID ${String(ready.metadata.supervisorOsIdentity.pid)}.\n`,
    );
    if (!options.noOpen) {
      (dependencies.openBrowser ?? openInDefaultBrowser)(ownerEvaluationBrowserSignInUrl());
    }
    emit(
      dependencies,
      `Open ${ownerEvaluationBrowserSignInUrl()}\nBEA_OWNER_EVALUATION_START=ALREADY_RUNNING\n`,
    );
    return {
  ...
  if (decision.action === "already-running") {
    emit(
      dependencies,
      `BEA Owner Evaluation is already running under authenticated supervisor PID ${String(tracked.metadata.supervisorOsIdentity.pid)}.\n`,
    );
    if (options.check) {
      emit(
        dependencies,
        `BEA_OWNER_EVALUATION_START_CHECK=PASS (RUNNING:${tracked.status.state})\n`,
      );
      return {
        outcome: "PASS",
        readOnly: true,
  ...
  if (decision.action === "clean-stale") {
    ...
    if (options.check) {
      emit(
        dependencies,
        "BEA_OWNER_EVALUATION_START_CHECK=BLOCKED (validated stale metadata requires cleanup)\n",
      );
  ...
  if (options.check) {
    ...
    if (lock.state === "live") {
      emit(dependencies, "BEA_OWNER_EVALUATION_START_CHECK=BLOCKED (LAUNCH_STARTING)\n");
      return { outcome: "BLOCKED", readOnly: true, reason: "LAUNCH_STARTING" };
    }
    if (lock.state === "stale") {
      emit(dependencies, "BEA_OWNER_EVALUATION_START_CHECK=BLOCKED (STALE_LAUNCH_LOCK)\n");
  ...
  if (options.check) {
    emit(
      dependencies,
      dependenciesPresent
        ? "BEA_OWNER_EVALUATION_START_CHECK=PASS\n"
        : "BEA_OWNER_EVALUATION_START_CHECK=PASS (SETUP_REQUIRED:FROZEN_INSTALL)\n",
    );
    return { outcome: "PASS", readOnly: true, setupRequired: !dependenciesPresent };
  }

  (dependencies.ensureDirectories ?? ensureOwnerEvaluationDirectories)(paths);
  ...
  try {
    ...
    if ((baseEnvironment.SESSION_SECRET ?? "").length < 64) {
      baseEnvironment.SESSION_SECRET = randomBytes(32).toString("hex");
    }
    const childEnvironment = createOwnerEvaluationChildEnvironment(baseEnvironment);
    ...
    runRequiredPnpm(
      ...
      childEnvironment,
      "Owner Evaluation database migration",
      dependencies,
    );
    ...
    if (shouldSeed) {
      runRequiredPnpm(
        ...
        childEnvironment,
        "Deterministic Owner Evaluation seed",
        dependencies,
      );
    } else {
      emit(
        dependencies,
        "==> Existing Owner Evaluation initialization found; explicit seed skipped.\n",
      );
    }

    const logs = (
      dependencies.createLogs ??
      (() => createRunLogPaths(new Date(), randomBytes, ownerEvaluationPaths.logDirectory))
    )();
    ...
    if (!supervisor?.pid) throw new Error("The Owner Evaluation supervisor did not return a PID.");
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `${error instanceof Error ? error.message : "Owner Evaluation supervisor identity capture failed."} Exact cleanup was refused: ${cleanupError instanceof Error ? cleanupError.message : "identity unavailable"}`,
          { cause: error },
        );
      }
      await verifyPortsAvailable(dependencies);
    ...
    try {
      ...
      emit(
        dependencies,
        "\nBEA Owner Evaluation is ready (Live OpenAI · Synthetic BEA Data · External Systems Not Connected).\n",
      );
      emit(dependencies, `Sign-in: ${ownerEvaluationBrowserSignInUrl()}\n`);
      emit(dependencies, `Supervisor PID: ${String(ready.metadata.supervisorOsIdentity.pid)}\n`);
      emit(
        dependencies,
        "PostgreSQL Local Live was not used. OpenAI uses a Windows protected key or a server runtime credential. Never open /api/auth/sign-in.\n",
      );
      if (!options.noOpen)
        (dependencies.openBrowser ?? openInDefaultBrowser)(ownerEvaluationBrowserSignInUrl());
      emit(dependencies, "BEA_OWNER_EVALUATION_START=PASS\n");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  void main().catch((error) => {
    process.stderr.write(
      `BEA_OWNER_EVALUATION_START=BLOCKED (${error instanceof Error ? error.message : "UnknownError"})\n`,
    );
    process.exitCode = 1;
  });
}
